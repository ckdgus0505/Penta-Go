...
pthread_mutex_t mutex;	// 쓰레드 뮤텍스

void *print_playtime(); // 쓰레드 실행할 함수, 시간을 출력하는 역할을 한다.
/* 클라이언트 플레이 함수 */
...
// 기보를 저장하기 위해 파일 디스크립터 지정
	...
	rcc = pthread_create(&t1, NULL, print_playtime, NULL);
...
void *print_playtime() {
  ...
  while (1) {
    ...
    printf("\33[%dd\033[%dG", 0, 30);
    printf("%02d : %02d", (now - start_time)/60, (now - start_time)%60);           printf("\33[%dd\033[%dG", 0, 40);
    sleep(0.9);
...
void print_board() {
	pthread_mutex_lock(&mutex); // 뮤텍스를 사용하여 상호배제 시킴
	printf("\33[%dd\033[%dG", 0, 0);
	printf(" │ A │ B │ C │ D │ E │ F │\n");
	printf("─┼───┼───┼───┼───┼───┼───┼\n");
	int i=0, j=0;
	for(int l = 0; l < 6; l++) {
...
int my_turn(int ns, char dol) {
	...
	char x, y, quad, c;
	int errcnt = 0; // 오류출력 위치를 잡을 변수

	pthread_mutex_lock(&mutex); 
	printf("\33[%dd\033[%dG", 3, 30);
	printf("좌표 (ex, A1) : ");
	pthread_mutex_unlock(&mutex);
	while(1) {
		__fpurge(stdin);
		...
		if (( (x >= 'A' && x <= 'F') || (x >= 'a' && x <= 'f'))
					&& y >= '1' && y <= '6') break;
			pthread_mutex_lock(&mutex); 
			printf("\33[%dd\033[%dG", 16 + errcnt++, 0);
	...
	pthread_mutex_lock(&mutex);
	printf("\33[%dd\033[%dG", 5, 30);
	printf("┌───┬───┐\n");
	printf("\33[%dd\033[%dG", 6, 30);
	printf("│ 1 │ 2 │\n");
	printf("\33[%dd\033[%dG", 7, 30);
	printf("├───┼───┤\n");
	printf("\33[%dd\033[%dG", 8, 30);
	printf("│ 3 │ 4 │\n");
	printf("\33[%dd\033[%dG", 9, 30);
	printf("└───┴───┘\n");
	printf("\33[%dd\033[%dG", 10, 30);
	printf("회전할 사분면\n");
	pthread_mutex_unlock(&mutex);
